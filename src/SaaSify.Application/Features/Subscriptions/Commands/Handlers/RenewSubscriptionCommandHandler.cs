...
public class RenewSubscriptionCommandHandler : IRequestHandler<RenewSubscriptionCommand, Result<SubscriptionResponse>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Solo se puede renovar una suscripción Active o PastDue.
        // Si está Cancelled o Expired, el developer debe asignar un nuevo plan.
        var subscription = await _subscriptionRepository
            .GetActiveByCustomerIdAsync(customer.Id, cancellationToken);

        if (subscription == null)
            return Result<SubscriptionResponse>.Failure(
                "No active subscription found. Use assign-plan to create a new one",
                errorCode: 404);

        // ── Paso 4: Buscar el plan para el mapeo ──────────────────────────
        // Se necesita el plan para devolver PlanName y PlanSlug en la response.
        ...
        // ── Paso 5: Renovar la suscripción ────────────────────────────────
        // Subscription.Renew() actualiza:
        //   - CurrentPeriodStart = ahora
        //   - CurrentPeriodEnd = ahora + 1 mes (o 1 año según BillingCycle)
        //   - RenewsAt = nuevo CurrentPeriodEnd
        //   - Status = Active
        subscription.Renew(request.ExternalPaymentRef);
